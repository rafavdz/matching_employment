...
sapply(grids, function(i) {
  ...
  zone_id <- unique(access_pt_grid$from_id)
  od_points_s <- od_points[[i]][od_points[[i]]$GEOID %in% zone_id, ]
  od_points_s <- od_points_s[order(od_points_s$GEOID), ]
...
access_pt_aggregated <- access_pt %>%
  group_by(grid, from_id, opportunity) %>%
  summarise(
...
sas_decriptive_stats <- access_pt_aggregated %>%
  ...
  group_by(opportunity, grid_labelled) %>%
  summarise(
  ...
  select(opportunity, SAS = grid_labelled, access_sum, N) %>%
  pivot_wider(names_from = opportunity, values_from = access_sum)

# N. observations below SAS 
n_observations <- access_pt_aggregated %>% 
  st_drop_geometry() %>% 
  ...
  pull(n)
sas_decriptive_stats$N <- n_observations

sas_decriptive_stats

# Save Table: desc. stats
write_csv(sas_decriptive_stats, "output/exploratory_rq1/sas_decriptive_stats.csv")
...
map1_title <- 
  "Median MPTN accessibility to employment by public transport. Greater Mexico City."
map1_subtitle <- 
  "Accessibility is computed as the median of the period of study for each spatial unit."
map1_fill <- 
  "Accessibility \ndecile"
map1_caption <- 
...
# access_pt %>% 
#   group_by(grid, opportunity, from_id) %>%
#   summarise(accessibility = median(accessibility, na.rm = TRUE)) %>%
#   pivot_wider(names_from = opportunity, values_from = accessibility) %>% 
#   group_by(grid) %>% 
#   summarise(
...
# Define CBD
cbd <- data.frame(X = -99.13311, Y =19.43252) %>% 
  st_as_sf(coords = c('X', 'Y'), crs = 4326) %>% 
  st_transform(crs = 6362)

# Calculate Eucludian distance to CBD from each OD
od_points_cbd <- od_points_all %>% 
  st_as_sf(coords = c('X', 'Y'), crs = 4326) %>%
  st_transform(crs = 6362) %>%
  mutate(
    dist_cbd = st_distance(geometry, cbd),
    dist_cbd = as.numeric(dist_cbd) / 1e3
  ) %>% 
  st_drop_geometry()

# Scatter plot
access_scatterplot <- access_pt_wide %>% 
  st_drop_geometry() %>%
  left_join(census, by = c('grid', 'from_id' = 'id')) %>% 
  arrange(desc(edu_quintile)) %>% 
  filter(!is.na(edu_quintile)) %>% 
  ggplot(
    aes(
      accessibility_med_All / 1e3, 
      accessibility_med_Matching / 1e3, 
      col = edu_quintile
    )) +
  ...
  labs(
    col = 'Education level',
    x = 'All accessibility (in thousands)',
    y = 'Matching accessibility (in thousands)'
  ) +
...
# Save plot
ggsave(
  "output/plots_maps/exploratory/access_scatterplot.png", 
  access_scatterplot,
  height = 5, 
  width = 8, 
  dpi = 400, 
...
lisa_composition_plot <- access_spat_stats %>% 
  st_drop_geometry() %>% 
  mutate(pysal = ifelse(p_folded_sim <= 0.1, as.character(pysal), NA)) %>%
  filter(!is.na(pysal)) %>%
  left_join(census, by = c('grid', 'from_id' = 'id')) %>%
  group_by(grid_labelled, pysal, edu_quintile) %>% 
  summarise(
    pop = sum(pob1)
  ) %>% 
  filter(!pysal %in% c('Low-High', 'High-Low')) %>%
  ggplot(aes(edu_quintile, pop, fill = pysal)) +
  ...
  labs(
    ...
    fill = 'Similarity rate LISA cluster'
  ) +
...
# Save plot
ggsave(
  "output/plots_maps/exploratory/lisa_composition_plot.png", 
  lisa_composition_plot,
  height = 5, 
  width = 8, 
  dpi = 400, 
  bg = "white"
