...
# Select 0.5 km grid and join census data
...
# Compute access changes over time ----------------------------------------
...
car_thresholds <- access_car %>%
  ...
  summarise(
    ...
    access_car25 = DescTools::Quantile(
      x = accessibility,
      weights = pob1,
      probs = 0.25
    )
...
# Plot labels
threshold_labs <- paste0('Under P', c(10, 15, 20, 25))
stage_labs3 <- paste0('S', 1:7, ': ', names(stage_labs))

# Quick changes summary
changes_summary <- access_change_census %>% 
  ...
  summarise(
    ...
    pop_under25 = sum(if_else(accessibility < access_car25, pob1, 0), na.rm = TRUE),
  ) %>% 
  ungroup() %>% 
...
# Plot using only one threshold
equity_overtime <- changes_summary %>% 
  # Limit it to percentile 
  filter(name == 'Under P15') %>% 
...
access_change_census %>% 
  group_by(router, opportunity, edu_quintile_w) %>% 
  summarise(
    pop_under10 = sum(if_else(accessibility < access_car10, pob1, 0), na.rm = TRUE),
    pop_under15 = sum(if_else(accessibility < access_car15, pob1, 0), na.rm = TRUE),
    pop_under20 = sum(if_else(accessibility < access_car20, pob1, 0), na.rm = TRUE),
  ...
  ggplot(aes(router, value, group = opportunity, col = opportunity)) +
  geom_line(lwd = 1.1) +
  facet_grid(edu_quintile_w ~ name) +
  theme_minimal() +
  scale_color_viridis_d(option = 'turbo', begin = 0.1, end = 0.9) +
  labs(
    x = 'MPTN stage',
...
# Sensitivity under various thresholds
# Plot using only one threshold
sensitivity_equity_overtime <- changes_summary %>% 
  ggplot(aes(router, value, group = opportunity, col = opportunity)) +
  geom_line(lwd = 1.1) +
  facet_wrap(~name) +
  theme_minimal() +
...
# Save plot
ggsave(
  filename =  "output/plots_maps/exploratory/sensitivity_equity_overtime.png", 
  plot = sensitivity_equity_overtime,
  height = 4, 
  width = 6, 
  dpi = 400, 
  bg = "white"
...
equity_summary <- access_change_census %>% 
  ...
  mutate(
    ...
    pop_benefited25 = lag(pop_under25) - pop_under25
  ) %>% 
  ungroup()
...
# Sensitivity analysis in equity ------------------------------------------

# Plot population taken out of lower threshold
sensitivity_equity_barplot <- equity_summary %>% 
  filter(router != 'Before 2010' & !is.na(edu_quintile)) %>% 
  select(router, opportunity, edu_quintile, starts_with('pop_benefited')) %>% 
  pivot_longer(starts_with('pop_ben')) %>% 
  mutate(
    edu_quintile = factor(edu_quintile, labels = 1:5),
    name = factor(name, labels = paste0('P', c(10, 15, 20, 25))),
    router = factor(router, labels = stage_labs2[-1]),
    value = value / 1e3
  ) %>%
  # Plot
...
# Save plot
ggsave(
  filename =  "output/plots_maps/exploratory/sensitivity_equity_barplot.png", 
  plot = sensitivity_equity_barplot,
  height = 5, 
  width = 8, 
  dpi = 400, 
  bg = "white"
